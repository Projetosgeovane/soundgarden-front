		...
		reservas.forEach((reserva, index) => {
			...
			conteudoResultado.innerHTML +=
				`<tr>
					...
					<td>
						<button id="${reserva._id}" nome="${reserva.owner_name}" email="${reserva.owner_email}" 
...
deletarSim.onclick = async function deletarReserva() {
	try {
		...
		const ingressosDisponiveis = reserva.number_tickets + reserva.event.number_tickets;
		...
		// console.log(requestOptionsPutEvent)
		const requisicao = await fetch(`${BASE_URL}/events/${idEvento}`, requestOptionsPutEvent)
